    ...
    public function task()
    {
        ...
        foreach ($tasks as $value) {
            ...
            if ($value["AssignedBy"] == $userInfo["UserID"] && empty($taskPerformers)) {
                ...
                $dateCreated = new DateTime($taskPerformers["DateStart"]);
                $deadline = $dateCreated->add(new DateInterval("P" . $taskPerformers["Deadline"] . "D"));

                array_push($taskList, [
                    "TaskName" => $value["TaskName"],
                    "Status" => $taskPerformers["Status"],
                    "Avatar" => $Users[0]["Avatar"],
                    "FullName" => $Users[0]["FullName"],
                    "Deadline" => $deadline->format("d-m-Y"),
                    "Progress" => $taskPerformers["Progress"]
                ]);
            }
        }
