...
class TaskController extends BaseController
{
    ...
    public function task()
    {
        ...
        foreach ($tasks as $value) {
            ...
            $position = $this->taskModel->getPositionByID($Users[0]["PositionID"]);
            $value["Position"]  = $position[0]["PositionName"];
            $department = $this->taskModel->getDepartmentByID($Users[0]["DepartmentID"]);
            $value["Department"]  = $department[0]["DepartmentName"];
            ...
            if ($value["AssignedBy"] == $userInfo["UserID"] && empty($taskPerformers)) {
                ...
                foreach ($taskPerformers as $value) {
                    ...
                    $Users = $this->taskModel->getUserByID($value["UserID"]);
                    $value["Avatar"] = $Users[0]["Avatar"];
                    $value["FullName"] = $Users[0]["FullName"];
                    $position = $this->taskModel->getPositionByID($Users[0]["PositionID"]);
                    $value["Position"]  = $position[0]["PositionName"];
