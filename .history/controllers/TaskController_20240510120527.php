    ...
    public function task()
    {
        ...
        foreach ($tasks as $value) {
            ...
            if ($value["AssignedBy"] == $userInfo["UserID"] && empty($taskPerformers)) {
                ...
                foreach ($taskPerformers as $value2) {
                    $dateCreated = new DateTime($value2["DateStart"]);
                    $value2["DateStart"] = $dateCreated->format("d-m-Y");

                    $deadline = $dateCreated->add(new DateInterval("P" . $value2["Deadline"] . "D"));
                    $value2["Deadline"] = $deadline->format("d-m-Y");

                    array_push($taskList, $value2);
                }
            }
        }
