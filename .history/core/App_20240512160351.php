...
class App {
    ...
    public function handleUrl() {
        ...
        unset($this->route['default_controller']);
        $urlRoute = trim($url, '/');
        if(!empty($this->route)) {
            foreach($this->route as $key => $value) {
               if(preg_match('~'. $key .'~is', $urlRoute)) {
                $url = preg_replace('~'.$key.'~is', $value, $urlRoute);
               }
            }
        }
        ...
        if(method_exists($this->controller, $this->action)) {
            echo call_user_func_array([$this->controller, $this->action], $this->params);
            die;
        } else {
            $this->loadError();
        }
