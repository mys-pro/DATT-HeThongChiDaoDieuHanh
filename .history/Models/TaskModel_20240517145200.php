...
class TaskModel extends BaseModel
{
    ...
    {
        mysqli_begin_transaction($this->connect, MYSQLI_TRANS_START_READ_ONLY);
        try {
            ...
            $taskReview = $this->addData("TaskPerformer", ["'{$data["taskID"]}'", "'{$data["taskReview"]}'", "NULL", "'{$data["deadlineReview"]}'", "''", "'0'", "'1'", "'Chưa thực hiện'", "''"]);
            if (!$taskReview) {
                mysqli_rollback($this->connect);
                return false;
            }
