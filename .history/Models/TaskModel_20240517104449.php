...
class TaskModel extends BaseModel
{
    ...
    {
        try {
            // Kiểm tra sự tồn tại của công việc trước khi xóa
            $stmt = $this->connect->prepare("SELECT COUNT(*) FROM Tasks WHERE TaskID = ?");
            $stmt->execute([$id]);
            $count = $stmt->fetchColumn();

            if ($count == 0) {
                // Không tìm thấy công việc có ID tương ứng, trả về false
                return false;
            }
            ...
            // Xóa công việc từ bảng Tasks
            $stmt = $this->connect->prepare("DELETE FROM Tasks WHERE TaskID = ?");
            $stmt->execute([$id]);

            // Xóa các bản ghi liên quan từ bảng TaskPerformers
            $stmt = $this->connect->prepare("DELETE FROM TaskPerformers WHERE TaskID = ?");
            $stmt->execute([$id]);
            $this->connect->commit();
            return true;
        } catch (Exception $e) {
