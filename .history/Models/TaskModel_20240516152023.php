...
class TaskModel extends BaseModel
{
    ...
    public function viewTask($id) {
        $sql = "SELECT t.TaskName, t.Description, t.Priority, t.Progress AS ProgressTask, t.Status AS StatusTask, t.Deadline AS DeadlineTask, u.FullName, p.PositionName, d.DepartmentName, tp.UserID, tp.Deadline AS DeadlineTaskPerformers, tp.Progress AS ProgressTaskPerformers, tp.Reviewer, tp.Status AS StatusTaskPerformers, tp.Comment 
        FROM Tasks t JOIN Users u ON t.AssignedBy = u.UserID JOIN Positions p ON u.PositionID = p.PositionID LEFT JOIN Departments d ON u.DepartmentID = d.DepartmentID JOIN TaskPerformers tp ON t.TaskID = tp.TaskID WHERE t.TaskID = ${id}";
        return $this->getData($sql);
    }
}
