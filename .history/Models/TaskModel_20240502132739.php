...
class TaskModel extends BaseModel
{
    ...
    {
        if ($departmentID != 0) {
            $sql = "SELECT t.TaskName, 
            CASE
                WHEN t.Priority = 1 THEN 'Thấp'
                WHEN t.Priority = 2 THEN 'Vừa'
                WHEN t.Priority = 3 THEN 'Cao'
            END AS Priority,
            d.DepartmentName, DATE_ADD(t.DateCreated, INTERVAL t.Deadline DAY) AS ExpectedDate, t.CompletionDate, t.Status FROM Tasks t JOIN TaskPerformers tp ON t.TaskID = tp.TaskID JOIN Users u ON tp.UserID = u.UserID JOIN Departments d ON u.DepartmentID = d.DepartmentID WHERE tp.Reviewer = 0 AND d.DepartmentID = ${departmentID}";
        } else {
            $sql = "SELECT t.TaskName, 
            CASE
                WHEN t.Priority = 1 THEN 'Thấp'
                ...
                WHEN t.Priority = 3 THEN 'Cao'
            END AS Priority,
            d.DepartmentName, DATE_ADD(t.DateCreated, INTERVAL t.Deadline DAY) AS ExpectedDate, t.CompletionDate, t.Status FROM Tasks t JOIN TaskPerformers tp ON t.TaskID = tp.TaskID JOIN Users u ON tp.UserID = u.UserID JOIN Departments d ON u.DepartmentID = d.DepartmentID WHERE tp.Reviewer = 0";
        }
        return $this->getData($sql);
    }
}
