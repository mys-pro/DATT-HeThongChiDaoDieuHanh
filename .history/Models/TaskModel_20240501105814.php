...
class TaskModel extends BaseModel
{
    ...
    public function getTotalTasksByName($name)
    {
        $sql = "SELECT COUNT(tp.TaskID) AS TotalTasks FROM TaskPerformers tp JOIN Users u ON tp.UserID = u.UserID JOIN Departments d ON u.DepartmentID = d.DepartmentID WHERE d.DepartmentName = '${name}'";
    ...
    public function statisticalByYEAR($priority = 0)
    {
        if ($priority > 0) {
            $sql = "SELECT d.DepartmentName, COUNT(tp.TaskID) AS 'Tổng công việc',
                    ...
                    FROM TaskPerformers tp JOIN Users u ON tp.UserID = u.UserID 
