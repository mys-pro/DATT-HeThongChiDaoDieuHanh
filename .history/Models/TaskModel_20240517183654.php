...
class TaskModel extends BaseModel
{
    ...
    {
        $sql = "SELECT t.TaskID, t.TaskName, t.Description, t.Priority, t.Progress AS ProgressTask, t.Status AS StatusTask, t.Deadline AS DeadlineTask, u.Avatar, u.FullName, p.PositionName, d.DepartmentName, tp.UserID, tp.Deadline AS DeadlineTaskPerformers, tp.Progress AS ProgressTaskPerformers, tp.Reviewer, tp.Status AS StatusTaskPerformers, tp.Comment, dc.DocumentID, dc.FileName, dc.FileSize, dc.File FROM Tasks t JOIN Users u ON t.AssignedBy = u.UserID JOIN Positions p ON u.PositionID = p.PositionID LEFT JOIN Departments d ON u.DepartmentID = d.DepartmentID LEFT JOIN TaskPerformers tp ON t.TaskID = tp.TaskID JOIN Documents dc ON t.TaskID = dc.TaskID WHERE t.TaskID = ${id}";
        return $this->getData($sql);
    }
