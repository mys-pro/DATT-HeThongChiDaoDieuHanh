...
class TaskModel extends BaseModel
{
    ...
    public function statisticalByYear($priority = 0)
    {
        if ($priority > 0) {
            $sql = "SELECT d.DepartmentName, COUNT(tp.TaskID) AS 'Tổng công việc',
                    ...
                    SUM(CASE WHEN DATEDIFF(NOW(), tp.DateStart) > tp.Deadline THEN 1 ELSE 0 END) AS 'Quá hạn' 
    ...
    public function getTaskPerformersById($id = 0) {
        $sql = "SELECT t.TaskName, tp.* FROM Tasks t JOIN TaskPerformers tp ON t.TaskID = tp.TaskID WHERE UserID = ${id}";
        return $this->getData($sql);
    }
}
