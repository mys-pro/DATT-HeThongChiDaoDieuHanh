...
class TaskModel extends BaseModel
{
    ...
    public function updateTask($id, $data = [])
    {
        mysqli_begin_transaction($this->connect);
        try {
            $sql = "UPDATE Tasks SET TaskName='{$data["name"]}', Description='{$data["description"]}', Priority='{$data["priority"]}', Status='Chưa thực hiện', Deadline='{$data["deadlineTask"]}' WHERE TaskID = {$id}";
            $this->_query($sql);

            date_default_timezone_set('Asia/Ho_Chi_Minh');
            $currentDate = date('Y-m-d');
            $this->addData("TaskPerformers", ["'{$data["taskID"]}'", "'{$data["taskPerformers"]}'", "'{$currentDate}'", "'{$data["deadlineTaskPerformers"]}'", '', '0', '0', 'Chưa thực hiện', '']);

            $this->addData("TaskPerformers", ["'{$data["taskID"]}'", "'{$data["taskReview"]}'", NULL, "'{$data["deadlineReview"]}'", '', '0', '1', 'Chưa thực hiện', '']);
