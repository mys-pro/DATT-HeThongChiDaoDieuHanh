...
class TaskModel extends BaseModel
{
    ...
    public function reportByYear($departmentID = 0)
    {
        if ($departmentID != 0) {
            ...
            END AS Priority,
            d.DepartmentName, DATE_ADD(t.DateCreated, INTERVAL t.Deadline DAY) AS ExpectedDate, t.CompletionDate, t.Status 
            ...
            WHERE tp.Reviewer = 0 AND d.DepartmentID = ${departmentID} AND YEAR(t.DateCreated) = YEAR(CURRENT_DATE)";
        } else {
            $sql = "SELECT t.TaskName, 
            CASE
                WHEN t.Priority = 1 THEN 'Thấp'
            ...
            d.DepartmentName, DATE_ADD(t.DateCreated, INTERVAL t.Deadline DAY) AS ExpectedDate, t.CompletionDate, t.Status 
            FROM Tasks t JOIN TaskPerformers tp ON t.TaskID = tp.TaskID JOIN Users u ON tp.UserID = u.UserID JOIN Departments d ON u.DepartmentID = d.DepartmentID 
            WHERE tp.Reviewer = 0 AND YEAR(t.DateCreated) = YEAR(CURRENT_DATE)";
        }
        return $this->getData($sql);
    }
}
