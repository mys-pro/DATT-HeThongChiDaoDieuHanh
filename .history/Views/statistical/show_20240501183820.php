    ...
    $('#priority-filter').change(function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                doughnutChart1.plugins.register({
                    beforeDatasetDraw: function(chart, args, pluginOptions) {
                        // args: { index, meta, dataset }
                        // Truy cập vào các thông tin của dataset
                        var dataset = args.dataset;

                        // Kiểm tra nếu đây là dataset mà bạn muốn thay đổi
                        if (dataset.label === 'Data') {
                            var ctx = chart.ctx;
                            var arc = chart.getDatasetMeta(args.index).data[args.index];
                            var centerX = arc._model.x;
                            var centerY = arc._model.y;
                            var innerRadius = arc.innerRadius;
                            var outerRadius = arc.outerRadius;

                            // Thực hiện logic tùy chỉnh tại đây
                            // Ví dụ: Vẽ một hình tròn mới bên trong biểu đồ doughnut
                            ctx.beginPath();
