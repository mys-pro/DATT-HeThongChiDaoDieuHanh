        ...
        $(document).ready(function() {
            ...
            function formatState(state) {
                ...
                if (state.element.value == 'null') {
                    var $state = $(
                        '<span><i class="select-icon bi ' + $(state.element).data('icon') + ' ms-3 me-2 text-secondary border border-2 rounded-circle d-flex align-items-center justify-content-center"></i> ' +
            ...
            function adjustSelectWidth() {
                ...
                select.find('option').each(function() {
                    var optionText = $(this).data('title');
                    var optionRole = $(this).data('role') || '';
                    var optionCompany = $(this).data('company') || '';
                    var optionFullText = optionText + ' ' + optionRole + ' ' + optionCompany;

                    var tempSpan = $('<span>').text(optionFullText).css({
                ...
                select.next('.select2-container').css('width', maxWidth + 50); // Thêm một chút padding
            }

            // Gọi hàm adjustSelectWidth khi khởi tạo Select2
            adjustSelectWidth();
        })
    </script>
</div>