<div class="content w-100 px-2 overflow-hidden">
    ...
    <script>
        $(document).ready(function() {
            ...
            // Khi mở dropdown, ẩn phần tử đã chọn nếu nó là option đầu tiên
            var selectedOption = $(this).find(':selected');
            if (selectedOption.index() === 0) {
                $select.data('select2').$container.find('.select2-selection__rendered').html('');
            }
        });

        $select.on('select2:close', function() {
            // Khi đóng dropdown, hiển thị lại phần tử đã chọn
