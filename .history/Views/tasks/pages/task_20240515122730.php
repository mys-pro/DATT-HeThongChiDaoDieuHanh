                        ...
                        <select name="" id="TaskPerformers" class="border-0">
                            <?php foreach ($data["departmentList"] as $department) : ?>
                                <optgroup label="<?= $department["DepartmentName"] ?>">
                                    <?php foreach($date["userList"] as $user): ?>
                                        <option value="<?= $user["UserID"] ?>" data-title="<?= $user["FullName"] ?>" data-role="<?= $user["PositonName"] ?>" data-avatar="data:image/jpeg;base64,<?= base64_encode($userInfo["Avatar"]) ?>">Nguyễn Thị Hương</option>
                                    <?php endforeach; ?>
                                </optgroup>
                            <?php endforeach; ?>
        ...
        $(document).ready(function() {
            ...
            function formatState(state) {
                ...
                var $state = $(
                    ...
                    '<p class="m-0 text-secondary lh-1" style="font-size: 12px">' + $(state.element).data('role') + '</p></div></div>'
                );
