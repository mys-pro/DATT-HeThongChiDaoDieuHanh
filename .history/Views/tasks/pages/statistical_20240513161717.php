<div class="content w-100 px-2 overflow-hidden">
            ...
            <ul class="filter-list list-unstyled m-0 p-2 d-flex position-relative">
                <li class="filter-item me-2">
                    <select id="priority-filter" class="w-auto form-select text-secondary bg-transparent border-secondary">
                        <option value="0" disabled selected hidden>Độ ưu tiên</option>
    ...
    <div class="row row-cols-xl-4 row-cols-md-3 row-cols-sm-2 row-cols-1 py-2 g-2">
        <?php foreach ($data['statistical'] as $index => $item) : ?>
            <div class="col">
                <canvas id="doughnutChart<?= $index + 1 ?>" class="bg-white p-2 shadow-sm w-100 h-100"></canvas>
            </div>
        <?php endforeach; ?>
    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            //============================================= Doughnut Chart =============================================//
            <?php foreach ($data['statistical'] as $index => $department) : ?>
                const doughnutData<?= $index + 1 ?> = {
                    labels: [
            ...
            <?php foreach ($data['statistical'] as $index => $department) : ?>
                const doughnutLabel<?= $index + 1 ?> = {
                    id: 'doughnutLabel<?= $index + 1 ?>',
                    beforeDatasetDraw(chart, args, pluginOptions) {
                        ...
                        ctx.fillText(totalTasks[<?= $index ?>], xCoor, yCoor - 15);
            ...
            <?php foreach ($data['statistical'] as $index => $department) : ?>
                const doughnutConfig<?= $index + 1 ?> = {
                    type: 'doughnut',
                    data: doughnutData<?= $index + 1 ?>,
                    options: {
                        maintainAspectRatio: false,
                        cutout: '70%',
                    ...
                    plugins: [doughnutLabel<?= $index + 1 ?>]
                };
            <?php endforeach; ?>

            <?php foreach ($data['statistical'] as $index => $department) : ?>
                var dChart<?= $index + 1 ?> = new Chart($('#doughnutChart<?= $index + 1 ?>'), doughnutConfig<?= $index + 1 ?>);
            <?php endforeach; ?>
            ...
            $('#btn-filter-statistical').click(function() {
                ...
                var url = window.location.origin + window.location.pathname + "?" + "priority=" + priority + "&date=" + date + "&dateStart=" + dateStart + "&dateEnd=" + dateEnd;
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        <?php foreach ($data['statistical'] as $index => $department) : ?>
                            dChart<?= $index + 1 ?>.data.datasets[0].data = doughnutChart[<?= $index ?>];
                            dChart<?= $index + 1 ?>.update();
                        <?php endforeach; ?>
                    },
                });
            });
