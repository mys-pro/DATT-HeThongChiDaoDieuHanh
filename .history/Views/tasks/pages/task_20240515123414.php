                                ...
                                <optgroup label="<?= $department["DepartmentName"] ?>">
                                    <?php foreach ($data["userList"] as $user) : ?>
                                        <?php if ($user["DepartmentID"] == $department["DepartmentID"]) : ?>
                                            <option value="<?= $user["UserID"] ?>" data-title="<?= $user["FullName"] ?>" data-role="<?= $user["PositionName"] ?>" data-company="<?= $user["PositionName"]  ?>" data-avatar="data:image/jpeg;base64,<?= base64_encode($user["Avatar"]) ?>"><?= $user["FullName"] ?></option>
                                        <?php endif; ?>
                                    <?php endforeach; ?>
                                </optgroup>
                            <?php endforeach; ?>
