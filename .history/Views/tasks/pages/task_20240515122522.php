                            ...
                            <?php foreach ($data["departmentList"] as $department) : ?>
                                <optgroup label="<?= $department["DepartmentName"] ?>">
                                    <?php foreach($date["userList"] as $user): ?>
                                        <option value="<?= $user["UserID"] ?>" data-title="<?= $user["FullName"] ?>" data-role="Tổng cục trưởng" data-company="MISA TaskGov" data-avatar="https://example.com/avatar1.png">Nguyễn Thị Hương</option>
                                    <?php endforeach; ?>
                                </optgroup>
                            <?php endforeach; ?>
